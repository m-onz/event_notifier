...
var checkOneWeekBefore = (event, ignoreYear = false) => {
  var today = moment();
  var eventDate = ignoreYear ? moment(event.date).year(today.year()) : moment(event.date)
...
var upcomingEventsInNext30Days = () => {
  ...
  events.forEach(event => {
    var today = moment()
    var eventDate = event.frequency === 'yearly' ? moment(event.date).year(today.year()) : moment(event.date)
    if (eventDate.isAfter(today) && eventDate.diff(today, 'days') <= 30) {
      notify(`${event.name}'s event is in ${eventDate.diff(today, 'days')} days.`)
    }
  })
}
